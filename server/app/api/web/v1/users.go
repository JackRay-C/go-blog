...
func (u *User) Get(c *gin.Context) (*response.Response, error) {
	...
	if one, err := u.userService.SelectOne(&po.User{ID: id}); err != nil {
...
func (u *User) GetUserInfo(c *gin.Context) (*response.Response, error)  {
	...
	// 3、获取当前用户的角色
	if err := u.userRoleService.SelectUserRoles(&po.User{ID: currentUserId.(int)}, &roles); err != nil {
		return nil, response.InternalServerError.SetMsg("%s", err)
	}
	...
	// 5、获取当前用户的信息
	user, err := u.userService.SelectOne(&po.User{ID: currentUserId.(int)})
	if err != nil {
		return nil, err
	}
