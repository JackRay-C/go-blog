...
func (u *User) List(c *gin.Context) (*response.Response, error) {
	...
	if err := u.userService.SelectAll(&p, &po.User{Active: int8(atoi)}); err != nil {
		return nil, response.InternalServerError.SetMsg("%s", err)
	}
...
func (u *User) Get(c *gin.Context) (*response.Response, error) {
	...
	one, err := u.userService.SelectOne(&po.User{ID: id})
	if err != nil {
		return nil, response.InternalServerError.SetMsg("%s", err)
	}
...
func (u *User) Delete(c *gin.Context) (*response.Response, error) {
	...
	if err := u.userService.DeleteOne(&po.User{ID: id}); err != nil {
