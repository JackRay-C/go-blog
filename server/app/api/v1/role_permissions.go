...
func (r *RolePermission) Get(c *gin.Context) (*response.Response, error)  {
	...
	if err := r.rolePermissionService.SelectRolePermissions(&domain.Role{ID: id}, &permissions); err != nil {
...
func (r *RolePermission) Put(c *gin.Context) (*response.Response, error)  {
	...
	// 4、修改角色权限列表
	if err := r.rolePermissionService.UpdateRolePermissions(&domain.Role{ID: id}, permissions.Permissions); err != nil {
		return nil, response.InternalServerError.SetMsg("%s", err)
	}
