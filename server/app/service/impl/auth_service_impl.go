...
func (a *AuthServiceImpl) ILogin(body *dto.LoginBody) (*dto.Token, error) {
	...
	if err != nil {
		global.Logger.Errorf("generate access token failed: %s", err)
		return nil, vo.FailedGenerateToken
	}
	// 设置accessToken过期时间
	if err := token.SetAccessTokenExpire(accessToken, global.Setting.App.AccessTokenExpire); err != nil {
		global.Logger.Errorf("set access token expire failed: %s", err)
		return nil, vo.FailedGenerateToken
	}

	refreshToken, err := token.GenerateRefreshToken(&token.Claims{UserId: user.ID, Username: user.Username})
	if err != nil {
		global.Logger.Errorf("generate refresh token failed: %s", err)
		return nil, vo.FailedGenerateToken
	}
	// 设置refreshToken过期时间
	if err := token.SetRefreshTokenExpire(refreshToken, global.Setting.App.RefreshTokenExpire); err != nil {
		global.Logger.Errorf("set refresh token expire failed: %s", err)
		return nil, vo.FailedGenerateToken
	}
	...
	global.Logger.Infof("login success: %s", body)
	return t2, nil
}
...
func (a *AuthServiceImpl) IRefreshToken(refreshToken string) (*dto.Token, error) {
	...
	return &dto.Token{
		AccessToken:  accessToken,
		RefreshToken: refreshToken,
		Expire:       global.Setting.App.AccessTokenExpire,
	}, nil
}
