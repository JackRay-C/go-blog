...
type Head struct {
	ID           int            `json:"id" gorm:"type:int;primary_key;autoIncrement;common:主键ID;"`   // 主键ID，博客的唯一ID
	RepositoryID int            `json:"repository_id" gorm:"type:int" form:"repository_id"`           // 当前存储库的id
	Visibility   int            `json:"visibility" gorm:"type:tinyint(2)"`                            // 是否公开 1、私有 2、公开
	Status       int            `json:"status" gorm:"type:tinyint(2)"`                                // 博客状态 1、已暂存 2、已提交 3、已发布
