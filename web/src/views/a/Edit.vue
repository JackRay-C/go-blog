  ...
  <div class="edit">
    <div class="header">
      <div class="edit-header-left">
        ...
        <div class="title">
          <span class="title-text" v-show="!post_title_edit">{{ title }}</span>
          ...
          <span v-if="id" class="date-text"
            >创建于{{ created_at | momentfmt("YYYY-MM-DD HH:mm:ss") }}</span
          >
        </div>
      </div>
...
import { getPost, addPost, putPost } from "@/api/post";
          ...
          getPost(this.id)
            .then((res) => {
              if (res.code === 200) {
                ...
                this.editor.setValue(this.markdown_content);
              }
            })
            .catch((err) => {
              console.log(err);
